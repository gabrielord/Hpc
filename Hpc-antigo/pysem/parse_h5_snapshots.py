...
def gradient(snp_forward, Mtilde, g_lambda, g_mu):
    dco_lamb = snp_forward.dset['Lamb'].T * Mtilde * g_lambda
    dco_mu = snp_forward.dset['Mu'].T * Mtilde * g_mu
    return dco_lamb, dco_mu

def misfit():
    pass

def calculate_alpha(lambda_k, mu_k, alpha_lambda, s_lambda_k, alpha_mu, s_mu_k, g_tilde_lambda_k, g_tilde_mu_k):
    while (misfit(lambda_k + alpha_lambda * s_lambda_k, mu_k + alpha_mu * s_mu_k) >= 
           misfit(lambda_k, mu_k) + 10**(-4) * (alpha_lambda * np.dot(s_lambda_k, g_tilde_lambda_k) + alpha_mu * np.dot(s_mu_k, g_tilde_mu_k))):
        alpha_lambda = .5 * alpha_lambda
        alpha_mu = .5 * alpha_mu
    return alpha_lambda, alpha_mu    
